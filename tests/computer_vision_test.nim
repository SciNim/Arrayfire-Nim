...
suite "computer_vision":
  ...
  test "harris":
    ...
    ixx = ixx.convolve2(gauss_filt)
    ixy = ixy.convolve2(gauss_filt)
    iyy = iyy.convolve2(gauss_filt)
    ...
    for y  in draw_len..<img_color.dims(0)-draw_len:
      for x in draw_len..<img_color.dims(1)-draw_len:
        if h_corners[int(x * corners.dims(0) + y)] > 1e5:
          ...
          img_color[y, mseq(x-draw_len, x+draw_len), 0] = 0
          img_color[y, mseq(x-draw_len, x+draw_len), 1] = 1
          img_color[y, mseq(x-draw_len, x+draw_len), 2] = 0
